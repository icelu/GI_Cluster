...
# fun call: 		## take expression matrix sample, samples and genes
...
sampleCols <- function( d,
  ...
  #cat("pRow = ", pRow, is.null( pRow ),(pRow < 1), "\n")
  if ( (!is.null(pRow)) &&
       ((pRow <= 1) || (! is.null( weightsRow ) ) ) ) {
    ## only sample the rows and generate a sub-matrix if we're sampling over the rows
    space = nrow(d)
    sampleN = floor(space*pRow)
    sampRows = sort( sample(space, sampleN, replace = FALSE, prob = weightsRow) )
    #cat("sampRows = ", sampRows, "\n")
...
clustering.1D <- function(data, clnum=3, algorithms=list('Ckmeans.1d.dp','densityMclust'),alparams=list(), pCol=1, pRow=1, weightsCol=NULL, weightsRow=NULL, reps=2, verbose="false", outdir="", ft=data){
  ...
  #print(str(clmem_list))
  cm_list = list()
  for(i in 1:reps){
    if (verbose=="true"){
      message("Iteration ", i)
    }
    # here feature is in col
    if(pCol<1 || pRow<1){
      samples = sampleCols(data, pCol=pCol, pRow=pRow, weightsCol=weightsCol, weightsRow=weightsRow)$subcols;
      #samples = sampleCols(data, pCol=1, pRow=1, weightsCol=NULL, weightsRow=NULL)$subcols
    }
    else{
